...
import { breadcrumbSchema, patientGuideSchema, serializeJsonLd } from '$lib/data/seo.js';
  const seoBreadcrumb = breadcrumbSchema([{ name: 'خانه', path: '' }, { name: 'زانو', path: '/orthopedics/knee' }, { name: 'درمان غیرجراحی مینیسک', path: '/orthopedics/knee/meniscus-non-surgical' }]);
  const seoPage = patientGuideSchema({ name: 'درمان غیرجراحی مینیسک', description: 'درمان غیرجراحی مینیسک؛ راهنمای بیمارمحور 4B Clinic با رویکرد تشخیص‌محور و شواهد محتاطانه.', path: '/orthopedics/knee/meniscus-non-surgical' });
...
<svelte:head>
  ...
  <meta property="og:url" content="https://4b-clinic.com/orthopedics/knee/meniscus-non-surgical" />
...
<article class="guide">
  ...
  <main>{#each sections as s}<section><h2>{s[0]}</h2>{#each s[1] as p}<p>{p}</p>{/each}</section>{/each}
    <section class="evidence-links"><h2>مطالعه مرتبط</h2><a href="/articles/acs-autologous-conditioned-serum">ACS و محیط التهابی مفصل ←</a><a href="/articles/prgf-growth-factors">PRGF و فاکتورهای رشد ←</a></section><aside><strong>مسیر بعدی</strong><p>انتخاب درمان به معاینه، شدت آسیب، تصویربرداری و هدف بیمار وابسته است.</p><a href="/orthopedics/knee">بررسی مسیر درمان زانو</a><a href="/contact">درخواست ارزیابی</a></aside>
  </main>
</article>
